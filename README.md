...
The capstone is **each agent's Commissioned Cipher Console**: their chosen cartridge in the slot (Caesar, the rotor, or the Unbreakable Wheel), their name and colors on the Ops Panel, encoding a family member's message live at the demo. Day 3 reads the console line by line; Day 4 breaks the seals and commissions it. (Target Lock, the game-week capstone, lives on in the repo history and in `target_lock_pygame.py`.) Ten side missions are live for agents who finish early. The first two open after Day 2: [The Cipher Grid](https://silvermanjonathan.github.io/Secret-Spy-Academy/side_mission_cipher_grid.html) stacks a parallel-racks substitution cipher with a column-read grid into a double lock, and [The Safecracker's Ledger](https://silvermanjonathan.github.io/Secret-Spy-Academy/side_mission_safecracker.html) cracks a safe by counting and by gambling, then searches the ledger with best-so-far. After Day 3, [Attack at Dawn](https://silvermanjonathan.github.io/Secret-Spy-Academy/side_mission_attack_at_dawn.html) opens the first side-mission window: the message in pixel letters, its coded twin as colored shapes, the key card on screen, and a sabotage pair that ends in a real traceback, read last line first.
...
side_mission_attack_at_dawn.html ← side mission: Attack at Dawn, the shape-cipher show (after Day 3)
attack_at_dawn_pygame.py         ← Attack at Dawn's engine (linked from its page)
