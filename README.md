...
| 2 | [The Cipher Room](https://silvermanjonathan.github.io/Secret-Spy-Academy/day2_cipher_room.html) | cryptography day, two benches. Morning, the Cipher Machine: paper wheel by hand, the rack, `==`/`!=`, the slot search, the shift (wrap met as a crash-and-repair), the tally, the Caesar wheel, decode, the key ceremony. Afternoon, the Codebreaker's Bench: `while` & the sentinel gate, the frequency leak, cracking all 27 keys, the double-shift clerk's error. Morning hinge is a legitimate floor landing | The Dead Drop + the Safe House door |
